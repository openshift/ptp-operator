			...
			It("is verifying WPC GM state based on metrics", func() {
				...
				By("checking DPLL phase state locked", func() {
					...
					*/
					...
					buf, _, _ := pods.ExecCommand(client.Client, fullConfig.DiscoveredClockUnderTestPod, pkg.PtpContainerName, []string{"curl", pkg.MetricsEndPoint})
					phaseStatusPattern := `openshift_ptp_frequency_status\{from="([^"]+)",iface="([^"]+)",node="([^"]+)",process="([^"]+)"\} (\d+)`
					...
					Expect(phaseStatusMap["dpll"]).To(BeTrue(), "Expected dpll frequency status to be Locked for GM")
				...
				By("checking GM clock state locked", func() {
					...
					*/
					...
					buf, _, _ := pods.ExecCommand(client.Client, fullConfig.DiscoveredClockUnderTestPod, pkg.PtpContainerName, []string{"curl", pkg.MetricsEndPoint})
					ret := clockStateByProcesses(buf.String())
...
func processRunning(input string) map[string]bool {
...
func clockStateByProcesses(input string) map[string]bool {
	// Regular expression pattern
